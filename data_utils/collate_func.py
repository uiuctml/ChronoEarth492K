...
def temporal_collate_fn(batch, modal='optical', transform=None, scale=None, crop_size=None, normalize_wv=False, wv_max=2365.20, wv_min=447.17):
    data_list = []
    timestamp_list = []
    spatial_resolutions = []
    channel_wv = None
    
    Ts = [example['num_frames'] for example in batch]
    T_max = max(Ts)
    ...
    for i, example in enumerate(batch):
        ...
        spatial_resolutions.append(example['spatial_resolution'])
            
        if not isinstance(example[modal], torch.Tensor):
            data = torch.tensor(example[modal])
        else:
            data = example[modal]
        
        T = example['num_frames']
        pad = torch.zeros(T_max - T, *data.shape[1:], dtype=data.dtype)
        data = torch.cat([data, pad], dim=0)
        mask[i, :T] = True
        data_list.append(data)
        timestamp_list.append(list(example['timestamp']) + [-1] * (T_max - T))
    ...
    return {
        modal: data,
        'mask': mask,
        'timestamps': torch.Tensor(timestamp_list, dtype=torch.int32),
        'num_frames': Ts,
        'spatial_resolution': spatial_resolutions,
        f'{modal}_channel_wv': channel_wv,
    }
    
def downstream_collate_fn(batch, modal='optical', transform=None, scale=None, crop_size=None, normalize_wv=False, wv_max=2365.20, wv_min=447.17):
    data_list = []
    label_list = []
    nondata_mask_list = []
    channel_wv = None
    ...
    for example in batch:
        ...
        if not isinstance(example['label'], torch.Tensor):
            label_list.append(torch.tensor(example['label']))
        else:
            label_list.append(example['label'])
            
        if not isinstance(example[modal], torch.Tensor):
            data_list.append(torch.tensor(example[modal]))
        else:
            data_list.append(example[modal])
        nondata_mask = example.get('nondata_mask', None)
        if nondata_mask is not None:
            nondata_mask_list.append(torch.tensor(nondata_mask))
        else:
            nondata_mask_list.append(None)
            
        timestamp = example.get('timestamp', None)
        if timestamp is not None:
            timestamp_list.append(torch.tensor(timestamp))
        else:
            timestamp_list.append(None)
            
        label_timestamp = example.get('label_timestamp', None)
        if label_timestamp is not None:
            label_timestamp_list.append(torch.tensor(label_timestamp))
        else:
            label_timestamp_list.append(None)
    ...
    return_dict = {
        ...
        'labels': torch.stack(label_list).squeeze(dim=1),
        f'{modal}_channel_wv': channel_wv,
        'spatial_resolution': spatial_resolution
    }
        
    if None not in timestamp_list and len(timestamp_list) > 0:
        return_dict['timestamps'] = torch.stack(timestamp_list)
    if None not in label_timestamp_list and len(label_timestamp_list) > 0:
        return_dict['label_timestamps'] = torch.stack(label_timestamp_list).unsqueeze(-1)
    
    return return_dict

# collate function for dataloader of single modal data
...
def vanilla_collate_fn(batch, modal='optical', transform=None, random_crop=False, scale=None, crop_size=None):
    data_list = []
