...
def NormalizeAll(optical=None, radar=None, optical_mean=None, optical_std=None, radar_mean=None, radar_std=None, zero_mean=False):
     # normalize
    def normalize(x, mean, std):
        ...
        if zero_mean:
            ...
            x_normalized = (x - mean[None, :, None, None]) / std[None, :, None, None]
            ...
            x_normalized = torch.clip(x_normalized, 0, 1)
...
def pretrain_transform(optical, spatial_resolution, crop_size=None, scale=None, optical_mean=None, optical_std=None, random_resize=False, zero_mean=False):
    # normalization
    optical, _ = NormalizeAll(optical, None, optical_mean, optical_std, zero_mean=zero_mean)
    
    # resize
    if scale is not None:
        if random_resize:
            b_scale = np.random.uniform(1, scale)
        else:
            b_scale = scale
        optical, radar = ResizeAll(optical, radar, b_scale, crop_size)
        try:
            example['spatial_resolution'] = example['spatial_resolution'] / b_scale
        except:
            pass
    
    # random crop
    if crop_size is not None:
        optical, _, _ = RandomCropAll(optical, None, None, crop_size)
    
    # horizontal flip
    if np.random.random() < 0.5:
        optical, _, _ = HorizontalFlipAll(optical, None, None)
    
    # vertical flip
    if np.random.random() < 0.5:
        optical, _, _ = VerticalFlipAll(optical, None, None)
    
    return optical, spatial_resolution
...
def segmentation_transform_one_sample(optical, radar, label, spatial_resolution, crop_size=None, scale=None, is_train=True, random_rotation=True, 
                                       optical_mean=None, optical_std=None, radar_mean=None, radar_std=None, random_crop=False, random_resize=False, zero_mean=False):
    ...
    # normalize
    optical, radar = NormalizeAll(optical, radar, optical_mean, optical_std, radar_mean, radar_std, zero_mean)
    
    if scale is not None:
        if random_resize and is_train:
            b_scale = np.random.uniform(1, scale)
        else:
            b_scale = scale
        optical, radar = ResizeAll(optical, radar, b_scale, crop_size)
        spatial_resolution = spatial_resolution / b_scale
...
def segmentation_transform(example, crop_size=None, scale=None, is_train=True, random_rotation=True, 
    ...
    for i in range(num_samples):
        ...
        optical_i, radar_i, label_i, spatial_resolution_i = segmentation_transform_one_sample(
            ...
            optical_std, radar_mean, radar_std, random_crop, random_resize, zero_mean
        )
...
def classification_transform(example, crop_size=None, scale=None, is_train=True, random_rotation=True, 
    ...
    for i in range(num_samples):
        ...
        optical_i, radar_i, spatial_resolution_i = classification_transform_one_sample(optical_i, radar_i, spatial_resolution_i, crop_size, scale, is_train, random_rotation,
                                                                                       optical_mean, optical_std, radar_mean, radar_std, random_crop, random_resize, zero_mean)
        if optical_i is not None:
            optical_list.append(optical_i)
        if radar_i is not None:
            radar_list.append(radar_i)
...
def get_transform(task_type, crop_size=None, scale=None, random_rotation=True, optical_mean=None, optical_std=None, radar_mean=None, radar_std=None, random_crop=False, random_resize=False, zero_mean=False):
    if task_type == "segmentation":
        train_transform = partial(segmentation_transform, crop_size=crop_size, scale=scale, random_rotation=random_rotation, is_train=True, 
                                  optical_mean=optical_mean, optical_std=optical_std, radar_mean=radar_mean, radar_std=radar_std, random_crop=random_crop, random_resize=random_resize, zero_mean=zero_mean)
...
def get_downstream_transform(task_type, crop_size=None, scale=None, random_rotation=True, optical_mean=None, optical_std=None, radar_mean=None, radar_std=None, random_crop=False, random_resize=False, zero_mean=False):
    if task_type in ["segmentation", "regression"]:
        train_transform = partial(segmentation_transform_one_sample, crop_size=crop_size, scale=scale, random_rotation=random_rotation, is_train=True, 
                                  optical_mean=optical_mean, optical_std=optical_std, radar_mean=radar_mean, radar_std=radar_std, random_crop=random_crop, random_resize=random_resize, zero_mean=zero_mean)
        eval_transform = partial(segmentation_transform_one_sample, crop_size=crop_size, scale=scale, is_train=False, 
                                  optical_mean=optical_mean, optical_std=optical_std, radar_mean=radar_mean, radar_std=radar_std, random_crop=random_crop, random_resize=random_resize, zero_mean=zero_mean)
    elif task_type in ["multilabel", "classification"]:
        train_transform = partial(classification_transform_one_sample, crop_size=crop_size, scale=scale, random_rotation=random_rotation, is_train=True, 
                                  optical_mean=optical_mean, optical_std=optical_std, radar_mean=radar_mean, radar_std=radar_std, random_crop=random_crop, random_resize=random_resize, zero_mean=zero_mean)
        eval_transform = partial(classification_transform_one_sample, crop_size=crop_size, scale=scale, is_train=False, 
                                  optical_mean=optical_mean, optical_std=optical_std, radar_mean=radar_mean, radar_std=radar_std, random_crop=random_crop, random_resize=random_resize, zero_mean=zero_mean)
    else:
        raise NotImplementedError
