...
def parse_args():
    ...
    parser.add_argument("--regions", type=str, nargs='+', default=['AC', 'AF', 'EA', 'EU', 'LA', 'NA', 'OC', 'SEA', 'SWA'], help="Regions to use for training")
    parser.add_argument("--channel_groups", type=str, nargs='+', default=['VNIR', 'SWIR1', 'SWIR2', 'SWIR3', 'SWIR4'], help="Channel groups to use for training, and order matters!")
    parser.add_argument("--num_frames", type=int, default=16, help="Temporal dataset specific arguments")
    parser.add_argument("--frames_lb", type=int, default=1, help="Temporal dataset specific arguments")
    parser.add_argument("--zero_mean", action="store_true", help="Whether to use zero mean for training")

    # Model arguments
    parser.add_argument("--model_name", type=str, choices=["lessvit", "leastvit"], default="lessvit", help="Choose between LESSViT and LEASTViT")
    ...
    # LEASTViT specific arguments
    parser.add_argument("--temporal_embed_dims_per_head", type=int, default=4, help="Number of temporal embedding dimensions per head")
    parser.add_argument("--temporal_mask_ratio", type=float, default=0.25, help="Temporal mask ratio for MAE")
    parser.add_argument("--encoder_attention_radius", type=int, nargs="+", default=[640], help="Attention radius for encoder")
    parser.add_argument("--decoder_attention_radius", type=int, nargs="+", default=[640], help="Attention radius for decoder")
    parser.add_argument("--use_temporal_causal_mask", action="store_true", help="Use temporal causal mask")
    parser.add_argument("--encoder_history_length", type=int, nargs="+", default=[4], help="History length for encoder")
    parser.add_argument("--decoder_history_length", type=int, nargs="+", default=[4], help="History length for decoder")
    parser.add_argument("--decoder_temporal_embed_dims_per_head", type=int, default=4, help="Number of temporal embedding dimensions per head for decoder")
