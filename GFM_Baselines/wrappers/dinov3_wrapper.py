from GFM_Baselines.models.DINOv3.dinov3_models import *
...
class DINOv3Encoder(DinoVisionTransformer):
    ...
    def forward_encoder(self, x, **kwargs):
        feature_all = super().forward(x=x, is_training=True)
        cls_token, features = feature_all['x_prenorm'][:, :1], feature_all['x_prenorm'][:, 1+self.n_storage_tokens:]
