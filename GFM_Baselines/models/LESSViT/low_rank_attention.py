...
@torch.no_grad()
def get_perception_field_mask(num_patches, patch_size, spatial_resolution, attention_radius, cls_token=False):
    s = int(math.sqrt(num_patches))
    assert s * s == num_patches, "num_patches must be a perfect square."

    # device = "cpu"
    # grid in pure torch
    y = torch.arange(s, device="cpu", dtype=torch.float32)
    x = torch.arange(s, device="cpu", dtype=torch.float32)
    yy, xx = torch.meshgrid(y, x, indexing="ij")
    pts = torch.stack([yy.flatten(), xx.flatten()], dim=-1)                        # [HW, 2]
    d = torch.cdist(pts, pts, p=2) * patch_size * spatial_resolution               # [HW, HW]
    mask = (d <= attention_radius)                                                 # bool [HW, HW]

    if cls_token:
        mask = torch.block_diag(torch.ones(1,1, dtype=torch.bool, device="cpu"), mask)
        mask[:1, 1:] = True
        mask[1:, :1] = True
    return mask  # [L,S] boolean
...
class LowRankAttention(nn.Module):
    ...
    def forward(self, x_c: torch.Tensor, x_s: torch.Tensor, spatial_mask: torch.Tensor = None, rope: tuple = None) -> torch.Tensor:
        ...
        x = torch.einsum('...ca,...nb->...cnab', xc, xs).flatten(-2).contiguous() # B, rank, num_heads, C, HW, D
