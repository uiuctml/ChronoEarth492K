...
class SpatialSpectralLowRankViTConfig(PretrainedConfig):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.num_patches = (input_size // patch_size) ** 2 if input_size is not None else None
        ...
        # Channel groups for channel masking
...
class SpatialSpectralLowRankViTEncoder(PreTrainedModel):
    ...
    def __init__(self, config: SpatialSpectralLowRankViTConfig):
        ...
        # Initialize perception field mask
        if self.config.use_perception_field_mask:
            self.register_buffer(
                'perception_field_mask', 
                torch.empty([self.config.num_patches, self.config.num_patches]), 
                persistent=True
            )

            mask = get_perception_field_mask(self.config.num_patches, self.config.patch_size, self.config.spatial_resolution, attention_radius=self.config.attention_radius, cls_token=False)
            self.perception_field_mask.copy_(mask)

        else:
            self.perception_field_mask = None

        self.num_channel_groups = config.num_channel_groups
    ...
    def forward(self, optical=None, radar=None, optical_channel_wv=None, radar_channel_wv=None, spatial_resolution=10, mask_ratio=None, channel_mask_ratio=None):
        ...
        if self.config.use_perception_field_mask:
            if num_patches != self.config.num_patches or spatial_resolution != self.config.spatial_resolution:
                mask = get_perception_field_mask(num_patches, self.config.patch_size, spatial_resolution, attention_radius=self.config.attention_radius, cls_token=False).to(x.device)
                self.perception_field_mask.copy_(mask)
        ...
        if not self.config.use_rope_embed:
            ...
            pos_chan_embed = self.pos_chan_embed(H=H, W=W, C=C, optical_channel_wv=optical_channel_wv)
        ...
        if self.training and self.config.pretrain:
            ...
            x_, pos_chan_embed_, channel_mask, channel_ids_restore = self.random_channel_masking(x_, pos_chan_embedding=pos_chan_embed_, channel_mask_ratio=channel_mask_ratio, use_rope_embed=self.config.use_rope_embed, channel_groups=self.num_channel_groups) # B N HW+1 D, B C, B C
            ...
            x_ = x[:, :, 1:, :] # B N+1 HW D
            pos_chan_embed_ = pos_chan_embed[:, :, 1:, :] if not self.config.use_rope_embed else pos_chan_embed
            x_, pos_chan_embed_, pos_mask, pos_ids_restore, perception_field_mask = self.random_pos_masking(x_, pos_chan_embedding=pos_chan_embed_, mask_ratio=mask_ratio, perception_field_mask=self.perception_field_mask, use_rope_embed=self.config.use_rope_embed) # B N+1 L D, B HW, B HW, L L
            ...
            perception_field_mask = self.perception_field_mask
        # assert False
    ...
    def random_channel_masking(self, x, pos_chan_embedding=None, channel_mask_ratio=0.5, batch_wise_mask=False, use_rope_embed=False, channel_groups=None):
        """
        Perform per-sample random masking by per-sample shuffling.
        Per-sample shuffling is done by argsort random noise.
        x: [B, C, HW, D], sequence
        ...
        assert sum(channel_groups) == C, "Sum of channel_groups must be equal to the number of channels C"

        x_masked_list = []
        pos_masked_list = [] if pos_chan_embedding is not None else None
        mask_list = []
        ids_restore_list = []
        ...
        for g in channel_groups:
            ...
            x_g = x[:, start:end, :, :]  # B, g, HW, D
            if pos_chan_embedding is not None and not use_rope_embed:
                pos_g = pos_chan_embedding[:, start:end, :, :]  # B, g, HW, D
            elif pos_chan_embedding is not None and use_rope_embed:
                pos_g = pos_chan_embedding
            else:
                pos_g = None
            
            len_keep = max(int(g * (1 - channel_mask_ratio)), 2) # at least 2 channel is kept, N

            # noise for shuffling
            if batch_wise_mask:
                noise = torch.rand(1, g, device=x.device).repeat(B, 1)
            else:
                noise = torch.rand(B, g, device=x.device)
            
            ids_shuffle = torch.argsort(noise, dim=1)  # [B, g]
            ids_restore = torch.argsort(ids_shuffle, dim=1)  # [B, g]

            ids_keep = ids_shuffle[:, :len_keep]  # [B, len_keep]
            x_masked_g = torch.gather(
                x_g, dim=1, 
                index=ids_keep.unsqueeze(-1).unsqueeze(-1).repeat(1, 1, HW, D)
            )  # [B, len_keep, HW, D]

            mask_g = torch.ones([B, g], device=x.device)
            mask_g[:, :len_keep] = 0
            mask_g = torch.gather(mask_g, dim=1, index=ids_restore)

            # handle positional embedding
            if pos_g is not None:
                if use_rope_embed:
                    pos_g2 = torch.gather(
                        pos_g[2][:, start:end], dim=1, 
                        index=ids_keep.unsqueeze(-1).repeat(1, 1, pos_g[2].shape[-1])
                    )
                    pos_g3 = torch.gather(
                        pos_g[3][:, start:end], dim=1, 
                        index=ids_keep.unsqueeze(-1).repeat(1, 1, pos_g[3].shape[-1])
                    )
                    pos_masked_g = [pos_g[0], pos_g[1], pos_g2, pos_g3]
                else:
                    pos_masked_g = torch.gather(
                        pos_g, dim=1, 
                        index=ids_keep.unsqueeze(-1).unsqueeze(-1).repeat(1, 1, HW, D)
                    )
            else:
            ...
            x_masked_list.append(x_masked_g)
            if pos_masked_list is not None:
                pos_masked_list.append(pos_masked_g)
            mask_list.append(mask_g)
            ids_restore_list.append(ids_restore + start)

            start = end
        
        # concat all groups back
        ...
        if pos_chan_embedding is not None:
            if use_rope_embed:
                pos_chan_embed_masked = [pos_masked_list[0][0], pos_masked_list[0][1], # since we didn't change anything for first two pos_embed
                                        torch.cat([g[2] for g in pos_masked_list], dim=1),
                                        torch.cat([g[3] for g in pos_masked_list], dim=1)]
            else:
                pos_chan_embed_masked = torch.cat(pos_masked_list, dim=1)
...
class SpatialSpectralLowRankViTDecoder(PreTrainedModel):
    ...
    def forward(self, x, pos_ids_restore, channel_ids_restore, channel_wv, spatial_resolution, restore_input_dim=False):
        # use the sptial cls tokens to represent all channels
        # x = x[:, 0] # B L+1 D
        # x = x[:, 1:].mean(dim=1) # B L+1 D
